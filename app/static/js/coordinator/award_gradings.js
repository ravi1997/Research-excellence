(function () {
  ...
  async function fetchAwards() {
    ...
    try {
      ...
      const response = await fetch(`${BASE}/api/v1/research/awards?${params.toString()}`, {
        headers: authHeaders(),
      });
      if (!response.ok) {
        throw new Error(`Failed to load awards ${response.status}`);
      }
      const data = await response.json();
      ...
      console.error("Unable to load awards", error);
      awardList.innerHTML =
        `<div class="p-5 text-sm text-red-600 dark:text-red-300">Failed to load awards. ${escapeHtml(error.message || "")}</div>`;
      state.awards = [];
      state.total = 0;
      updatePagination();
    } finally {
  ...
  function renderAwards() {
    ...
    if (state.isLoading) {
      awardList.innerHTML =
        `<div class="p-5 text-sm text-gray-500 dark:text-gray-400 animate-pulse">Loading awards...</div>`;
      return;
    }

    if (!state.awards.length) {
      awardList.innerHTML =
        `<div class="p-5 text-sm text-gray-500 dark:text-gray-400">No awards match the current filters.</div>`;
    ...
    const rows = state.awards
      .map((award) => {
        ...
        const number = award.award_number ? `#${award.award_number}` : "—";
            ...
            <div class="px-5 py-4 space-y-2">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h3 class="text-base font-semibold text-gray-900 dark:text-white line-clamp-2">${escapeHtml(award.title || "Untitled award")}</h3>
                </div>
  ...
  async function fetchGradings(awardId) {
    ...
    gradingPanel.innerHTML = `
      <div class="space-y-3">
        <div>
          <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Award</p>
          <p class="text-base font-semibold text-gray-900 dark:text-white">${escapeHtml(selected?.title || "Untitled award")}</p>
        </div>
    ...
    try {
      const response = await fetch(`${BASE}/api/v1/research/awards/${awardId}/gradings`, {
        headers: authHeaders(),
      });
      if (!response.ok) {
      ...
      gradingPanel.innerHTML = `
        <div class="space-y-3">
          ...
          <div class="rounded-2xl border border-dashed border-gray-300 p-4 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
            No gradings have been submitted for this award yet.
          </div>
        </div>
      `;
      return;
  ...
  function awardDetailsMarkup(award, cycleName) {
    ...
    return `
      <div class="space-y-2">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">${escapeHtml(award.title || "Untitled award")}</h3>
