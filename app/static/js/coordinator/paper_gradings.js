(function () {
  ...
  const awardList = document.getElementById("bestpaperList");
  const awardCount = document.getElementById("bestpaperCount");
  ...
  function init() {
    bindEvents();
    fetchCycles().finally(fetchAwards);
  }
  ...
  function bindEvents() {
    ...
    exportButton?.addEventListener("click", () => exportBestPapers());
    ...
    awardList?.addEventListener("click", (event) => {
      const row = event.target.closest("[data-award-id]");
      if (!row) return;
  ...
  async function fetchAwards() {
    ...
    try {
      ...
      const response = await fetch(`${BASE}/api/v1/research/best-papers?${params.toString()}`, {
        headers: authHeaders(),
      });
      if (!response.ok) {
        throw new Error(`Failed to load best papers ${response.status}`);
      }
      const data = await response.json();
      ...
      console.error("Unable to load best papers", error);
      awardList.innerHTML =
        `<div class="p-5 text-sm text-red-600 dark:text-red-300">Failed to load best papers. ${escapeHtml(error.message || "")}</div>`;
      state.awards = [];
      state.total = 0;
      updatePagination();
    } finally {
  ...
  function renderAwards() {
    ...
    if (state.isLoading) {
      awardList.innerHTML =
        `<div class="p-5 text-sm text-gray-500 dark:text-gray-400 animate-pulse">Loading best papers...</div>`;
      return;
    }

    if (!state.awards.length) {
      awardList.innerHTML =
        `<div class="p-5 text-sm text-gray-500 dark:text-gray-400">No best papers match the current filters.</div>`;
      awardCount && (awardCount.textContent = "0");
      return;
    }

    awardCount && (awardCount.textContent = String(state.total));

    const rows = state.awards
      .map((award) => {
        const isSelected = state.selectedAwardId === String(award.id);
        const cycleName = state.cycles.get(String(award.cycle_id))?.name || "Unassigned";
        const badge = statusBadge(award.status);
        const category = award.category?.name || "Uncategorized";
        const number = award.bestpaper_number
          ? `#${award.bestpaper_number}`
          : award.award_number
            ? `#${award.award_number}`
            : "—";
        const reviewers = (award.verifiers_count ?? ((award.verifiers || []).length)) || 0;
        const phase = award.review_phase ?? "—";
        return `
          <button data-award-id="${escapeAttr(award.id)}" class="w-full text-left transition ${isSelected ? "bg-blue-50 dark:bg-blue-500/10" : "hover:bg-gray-50 dark:hover:bg-gray-800/40"} focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
            <div class="px-5 py-4 space-y-2">
              <div class="flex items-start justify-between gap-3">
                <div>
                 <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">${escapeHtml(number)} · ${escapeHtml(category)}</p>
                  <h3 class="text-base font-semibold text-gray-900 dark:text-white line-clamp-2">${escapeHtml(award.title || "Untitled best paper")}</h3>
                </div>
              ...
              <dl class="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400">
                ...
                <div>
                  ...
                  <dd>${formatDate(award.updated_at) || "—"}</dd>
                </div>
              </dl>
            </div>
          </button>`;
  ...
  function selectAward(awardId) {
    if (!awardId) return;
    if (state.selectedAwardId === awardId) return;
    state.selectedAwardId = awardId;
    renderAwards();
    fetchGradings(awardId);
  }

  async function fetchGradings(awardId) {
    if (!gradingPanel) return;
    const selected = state.awards.find((item) => String(item.id) === String(awardId));
    gradingPanel.innerHTML = `
      <div class="space-y-3">
        <div>
          <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Best Paper</p>
          <p class="text-base font-semibold text-gray-900 dark:text-white">${escapeHtml(selected?.title || "Untitled best paper")}</p>
        </div>
        <p class="text-sm text-gray-500 dark:text-gray-400">Loading gradings…</p>
      </div>
    `;

    if (state.gradingCache.has(awardId)) {
      renderGradings(state.gradingCache.get(awardId), selected);
      return;
    }

    try {
      const response = await fetch(`${BASE}/api/v1/research/best-papers/${awardId}/gradings`, {
        headers: authHeaders(),
      });
      if (!response.ok) {
      ...
      state.gradingCache.set(awardId, gradings);
  ...
  async function exportBestPapers() {
    ...
    try {
      ...
      const url = `${BASE}/api/v1/research/best-papers/export-with-grades?${params.toString()}`;
      ...
      let filename = "bestpapers-grades.xlsx";
      ...
      alert("Failed to export best papers: " + (err.message || "unknown error"));
    } finally {
  ...
  function renderGradings(gradings, award) {
    ...
    if (!gradings || !gradings.length) {
      gradingPanel.innerHTML = `
        <div class="space-y-3">
          ${awardDetailsMarkup(award, cycleName)}
          <div class="rounded-2xl border border-dashed border-gray-300 p-4 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
            No gradings have been submitted for this best paper yet.
          </div>
        </div>
      `;
      return;
  ...
  function awardDetailsMarkup(award, cycleName) {
    if (!award) return "";
    const category = award.category?.name || "Uncategorized";
    const status = statusBadge(award.status);
    return `
      <div class="space-y-2">
        ...
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">${escapeHtml(award.title || "Untitled best paper")}</h3>
        <div class="flex flex-wrap gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-gray-700 dark:bg-gray-800 dark:text-gray-200">Phase ${escapeHtml(String(award.review_phase ?? "—"))}</span>
          <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-gray-700 dark:bg-gray-800 dark:text-gray-200">${(award.verifiers_count ?? ((award.verifiers || []).length)) || 0} verifier(s)</span>
        </div>
      </div>
    `;
  }
