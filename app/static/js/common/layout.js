...
(() => {
  ...
  if (isCordinator) {
    ...
    const verifierToggle = document.getElementById('verifierMenuToggle');
    const verifierPanel = document.getElementById('verifierMenuPanel');
    const verifierChevron = document.getElementById('verifierChevron');
    if (verifierToggle && verifierPanel) {
      verifierToggle.addEventListener('click', (e) => {
        e.preventDefault();
        const open = !verifierPanel.classList.contains('hidden');
        if (open) {
          verifierPanel.classList.add('hidden');
          verifierToggle.setAttribute('aria-expanded', 'false');
          if (verifierChevron) verifierChevron.style.transform = 'rotate(0deg)';
        } else {
          verifierPanel.classList.remove('hidden');
          verifierToggle.setAttribute('aria-expanded', 'true');
          if (verifierChevron) verifierChevron.style.transform = 'rotate(90deg)';
        }
      });
    }
