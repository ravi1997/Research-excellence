...
(function () {
    ...
    const next3 = document.querySelector('.next-step[data-step="3"]');
        ...
        if (next1) next1.addEventListener('click', function () {
            ...
            if (!paper_category || !paper_category.value) {
                ...
                paper_category && paper_category.focus();
                return;
            }
        ...
        if (next3) next3.addEventListener('click', function () {
            // Ensure AIIMS question is answered
            const yes = document.getElementById('is_aiims_work_yes');
            const no = document.getElementById('is_aiims_work_no');
            const err = document.getElementById('aiims-error');
            if (!yes || !no) { hide(step3); show(step4); mark(4); return; }
            if (!yes.checked && !no.checked) {
                if (err) { err.textContent = 'Please select Yes or No.'; err.classList.remove('sr-only'); }
                showToast('Please answer whether the work was primarily done at AIIMS.', 'error');
                return;
            }
            if (err) { err.textContent = ''; err.classList.add('sr-only'); }
    ...
    const pdfInput = document.getElementById('bestpaper_pdf') || document.getElementById('best_paper_pdf');
        ...
        // Handle Forwarding Letter PDF selection (Step 3)
        const fwdPdfInput = document.getElementById('forwarding_pdf');
        if (fwdPdfInput) {
            fwdPdfInput.addEventListener('change', function () {
                const file = this.files && this.files[0];
                const preview = document.getElementById('forwarding-pdf-preview');
                if (!file) { if (preview) preview.classList.add('hidden'); const e = document.getElementById('forwarding-pdf-error'); if (e) { e.textContent=''; e.classList.add('sr-only'); } return; }
                if (!validateForwardingPdf(file, this)) { this.value = ''; return; }
                updateForwardingPdfPreview(file);
            });
        }
        ...
        // Handle Forwarding PDF removal
        const removeFwdPdfBtn = document.getElementById('remove-forwarding-pdf');
        if (removeFwdPdfBtn) {
            removeFwdPdfBtn.addEventListener('click', function () {
                const input = document.getElementById('forwarding_pdf');
                const preview = document.getElementById('forwarding-pdf-preview');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () {
            setupFlow();
            setupEvents();
            // Editors removed from step 3; keep any other initializations minimal
        });
    } else {
        setupFlow();
        setupEvents();
        // Editors removed from step 3
    }
})();
...
// Step 3 no longer has content editors; counter not needed
...
// Content editors removed for Step 3; CKEditor related functions not used on this page now
...
async function fetchPaperCategories() {
    const select = document.getElementById('paper_category');
    if (!select) return;

    try {
        // Show loading state
        select.innerHTML = '<option value="">Loading paper categories...</option>';
        select.disabled = true;

        const response = await fetch(`${BASE_API_URL}/papercategories`, {
            method: 'GET',
            headers: {
...
function collectFormData() {
    ...
    // From Step 3: AIIMS question
    const aiimsYes = document.getElementById('is_aiims_work_yes');
    const aiimsNo = document.getElementById('is_aiims_work_no');
    data.is_aiims_work = aiimsYes && aiimsYes.checked ? true : aiimsNo && aiimsNo.checked ? false : null;
    

    // Single author (default corresponding)
    data.authors = [];
    const nameInput = document.getElementById('author-name');
    const emailInput = document.getElementById('author-email');
    const affiliationInput = document.getElementById('author-affiliation');
    const name = nameInput ? nameInput.value.trim() : '';
    const email = emailInput ? emailInput.value.trim() : '';
...
function generatePreview() {
    ...
    if (paperCategorySelect && paperCategorySelect.options[paperCategorySelect.selectedIndex]) {
        paperCategoryName = paperCategorySelect.options[paperCategorySelect.selectedIndex].text;
    }
    ...
    if (fwdPdfInput && fwdPdfInput.files[0]) {
        const file = fwdPdfInput.files[0];
        // Create a container for the PDF preview
        forpdfPreview = `
            <div class="mt-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div class="flex items-center justify-between mb-3">
    ...
    previewHTML += `
            ...
            <div class="p-6">
                ...
                <div class="ml-2 space-y-3">
                    <div class="flex">
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 w-56">Work done at AIIMS</dt>
                        <dd class="text-sm text-gray-900 dark:text-white">${formData.is_aiims_work === true ? 'Yes' : formData.is_aiims_work === false ? 'No' : 'Not answered'}</dd>
                    </div>
                    <div class="ml-2">
                            ${forpdfPreview || `<p class="text-gray-500 dark:text-gray-400 italic">No PDF file uploaded</p>`}
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="flex items-center mb-4">
                    ...
                    <div class="ml-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Full Paper</h3>
...
// Forwarding PDF helpers
function validateForwardingPdf(file, inputEl) {
    const errEl = document.getElementById('forwarding-pdf-error');
    const maxMB = Number(inputEl?.dataset?.maxSizeMb || 5);
    const isPdf = file.type === 'application/pdf' || /.pdf$/i.test(file.name || '');
    const sizeOk = file.size <= maxMB * 1024 * 1024;
    let msg = '';
    ...
    if (msg) {
        if (errEl) { errEl.textContent = msg; errEl.classList.remove('sr-only'); }
        showToast(msg, 'error');
        const p = document.getElementById('forwarding-pdf-preview');
...
function updateForwardingPdfPreview(file) {
    const preview = document.getElementById('forwarding-pdf-preview');
    const fileName = document.getElementById('forwarding-pdf-file-name');
    const fileSize = document.getElementById('forwarding-pdf-file-size');
    if (preview && fileName && fileSize) {
        fileName.textContent = file.name;
        fileSize.textContent = `Size: ${(file.size / 1024 / 1024).toFixed(2)} MB`;
        preview.classList.remove('hidden');
...
function renderPdfPreview(file, containerId = 'pdf-preview-container', canvasId = 'pdf-canvas') {
    // Check if PDF.js is loaded
    if (typeof pdfjsLib === 'undefined') {
        console.error('PDF.js library not loaded');
        showPdfPreviewError('PDF.js library not available', containerId, file?.name);
        return;
    }
    ...
    fileReader.onload = function () {
        ...
        loadingTask.promise.then(function (pdf) {
            ...
            renderPdfPage(pdf, file, containerId, canvasId);
        }).catch(function (error) {
            console.error('Error loading PDF document:', error);
            // Handle specific error types
            if (error.name === 'PasswordException') {
                // For password-protected PDFs, we can't show a preview
                showPdfPreviewError('Password-protected PDFs cannot be previewed for security reasons. Your file will still be submitted correctly.', containerId, file?.name);
            } else if (error.name === 'InvalidPDFException') {
                showPdfPreviewError('Invalid PDF file or corrupted document', containerId, file?.name);
            } else if (error.name === 'MissingPDFException') {
                showPdfPreviewError('PDF file not found', containerId, file?.name);
            } else {
                showPdfPreviewError('Unable to load PDF document: ' + (error.message || 'Unknown error'), containerId, file?.name);
            }
        });
    };
    ...
    fileReader.onerror = function () {
        ...
        showPdfPreviewError('Error reading PDF file', containerId, file?.name);
    };
...
function renderPdfPage(pdf, file, containerId = 'pdf-preview-container', canvasId = 'pdf-canvas') {
    ...
    pdf.getPage(1).then(function (page) {
        ...
        if (!canvas || !container) {
            showPdfPreviewError('PDF preview container not found', containerId, file?.name);
            return;
        }
    ...
    const context = canvas.getContext('2d');
        ...
        page.render(renderContext).promise.then(function () {
            ...
            showPdfPreviewError('Error rendering PDF preview', containerId, file?.name);
        });
    }).catch(function (pageError) {
        console.error('Error loading PDF page:', pageError);
        showPdfPreviewError('Error loading PDF page', containerId, file?.name);
    });
}

// Show PDF preview error message
function showPdfPreviewError(message, containerId = 'pdf-preview-container', fileName = 'PDF Document') {
    const container = document.getElementById(containerId);
    if (container) {
        container.innerHTML = `
            <div class="p-6 text-center">
                ...
                <div class="mt-4">
                    <div class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100">
                        ...
                        ${escapeHtml(fileName || 'PDF Document')}
                    </div>
                </div>
            </div>
        `;
