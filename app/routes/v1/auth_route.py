    ...
    if password:
        ...
        if identifier:
            ...
            if not user.is_verified:
                ...
                return _htmx_or_json_error("Account verification pending", 403)
...
def create_account():
    ...
    if user.is_verified:
        audit_log('create_account_failed', target_user_id=user.id, detail='already_verified')
        return jsonify({'msg': 'account already exist'}), 400
...
# Grant user role to a user (admin-only convenience endpoint)
@auth_bp.post('/grant-user')
@require_roles(Role.ADMIN.value, Role.SUPERADMIN.value)
def grant_user():
    """Grant the 'user' role to a specific user.
    Body: {"user_id": "<uuid>"}
    ...
    try:
        ...
        audit_log('grant_user_failed', target_user_id=target_id, detail='db_commit_failed')
        return jsonify({'msg': 'internal error'}), 500
    audit_log('grant_user_success', target_user_id=target_id)
...
def forgot_password():
    ...
    if not user.is_verified:
        current_app.logger.warning(
            f"Login failed: user {email or mobile} not verified by admin")
        audit_log('forgot_password_failed',
                  target_user_id=user.id, detail='not_verified')
        return _htmx_or_json_error("Account verification pending", 403)
