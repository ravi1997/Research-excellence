...
from app.models.Cycle import Abstracts, AbstractCoordinators
...
@research_bp.route('/abstract_coordinators', methods=['POST'])
@jwt_required()
@require_roles(Role.ADMIN.value, Role.SUPERADMIN.value)
def create_abstract_coordinator():
    """Assign a coordinator to an abstract."""
    actor_id, context = _resolve_actor_context("create_abstract_coordinator")
    ...
    try:
        ...
        if not isinstance(payload, dict):
            ...
            log_audit_event(
                event_type="abstract_coordinator.create.failed",
                user_id=actor_id,
        ...
        existing_assoc = AbstractCoordinators.query.filter_by(
            abstract_id=payload['abstract_id'],
            user_id=payload['user_id']
        ).first()
        if existing_assoc:
            error_msg = "Coordinator is already assigned to this abstract"
            log_audit_event(
                event_type="abstract_coordinator.create.failed",
                user_id=actor_id,
                details={
        ...
        abstract_coordinator = AbstractCoordinators(
            abstract_id=payload['abstract_id'],
            user_id=payload['user_id']
        )
        db.session.add(abstract_coordinator)
        db.session.commit()

        # Log successful creation
        log_audit_event(
            event_type="abstract_coordinator.create.success",
            user_id=actor_id,
            details={
                "abstract_id": payload['abstract_id'],
                "user_id": payload['user_id']
        ...
        return jsonify({
            "message": "Coordinator assigned to abstract successfully",
            "abstract_id": payload['abstract_id'],
            "user_id": payload['user_id']
        }), 201
    except Exception as exc:
        db.session.rollback()
        current_app.logger.exception("Error creating abstract coordinator association")
        error_msg = f"System error occurred while creating abstract coordinator association: {str(exc)}"
        log_audit_event(
            event_type="abstract_coordinator.create.failed",
            user_id=actor_id,
...
@research_bp.route('/abstract_coordinators/<abstract_id>/<user_id>', methods=['DELETE'])
@jwt_required()
@require_roles(Role.ADMIN.value, Role.SUPERADMIN.value)
def delete_abstract_coordinator(abstract_id, user_id):
    """Remove a coordinator from an abstract."""
    actor_id, context = _resolve_actor_context("delete_abstract_coordinator")
    
    try:
        # Find the specific association
        abstract_coordinator = AbstractCoordinators.query.filter_by(
            abstract_id=abstract_id,
            user_id=user_id
        ).first()
        
        if not abstract_coordinator:
            error_msg = f"Resource not found: Association between abstract ID {abstract_id} and user ID {user_id} does not exist"
            log_audit_event(
                event_type="abstract_coordinator.delete.failed",
                user_id=actor_id,
        ...
        db.session.delete(abstract_coordinator)
        db.session.commit()

        # Log successful deletion
        log_audit_event(
            event_type="abstract_coordinator.delete.success",
            user_id=actor_id,
            details={
                "abstract_id": abstract_id,
                "user_id": user_id
        ...
        error_msg = f"System error occurred while deleting abstract coordinator association: {str(exc)}"
        log_audit_event(
            event_type="abstract_coordinator.delete.failed",
            user_id=actor_id,
...
@research_bp.route('/abstracts/<abstract_id>/coordinators', methods=['GET'])
@jwt_required()
def get_coordinators_by_abstract(abstract_id):
    """Get all coordinators associated with a specific abstract."""
    actor_id, context = _resolve_actor_context("get_coordinators_by_abstract")
    ...
    try:
        ...
        if not abstract:
            ...
            log_audit_event(
                event_type="abstract_coordinators.list.failed",
                user_id=actor_id,
        ...
        # Get all coordinators associated with this abstract
        abstract_coordinators = AbstractCoordinators.query.filter_by(abstract_id=abstract_id).all()
        
        # Get the actual user details for each association
        coordinators_data = []
        for ac in abstract_coordinators:
        ...
        log_audit_event(
            event_type="abstract_coordinators.list.success",
            user_id=actor_id,
            details={
                "abstract_id": abstract_id,
        ...
        current_app.logger.exception("Error retrieving coordinators for abstract")
        error_msg = f"System error occurred while retrieving coordinators for abstract: {str(exc)}"
        log_audit_event(
            event_type="abstract_coordinators.list.failed",
            user_id=actor_id,
...
@research_bp.route('/coordinators/<user_id>/abstracts', methods=['GET'])
@jwt_required()
def get_abstracts_by_coordinator(user_id):
    """Get all abstracts assigned to a specific coordinator."""
    actor_id, context = _resolve_actor_context("get_abstracts_by_coordinator")
    ...
    try:
        ...
        if not privileged and str(actor_id) != user_id:
            error_msg = f"Authorization failed: You are not authorized to access coordinator ID {user_id}'s abstracts"
            log_audit_event(
                event_type="coordinator_abstracts.list.failed",
                user_id=actor_id,
                details={
        ...
        # Get all abstract-coordinator associations for this user
        abstract_coordinators = AbstractCoordinators.query.filter_by(user_id=user_id).all()
        
        # Get the actual abstract details for each association
        abstracts_data = []
        for ac in abstract_coordinators:
            abstract = abstract_utils.get_abstract_by_id(ac.abstract_id, actor_id=actor_id, context=context)
            if abstract:
        ...
        # Log successful retrieval
        log_audit_event(
            event_type="coordinator_abstracts.list.success",
            user_id=actor_id,
            details={
                "user_id": user_id,
        ...
        current_app.logger.exception("Error retrieving abstracts for coordinator")
        error_msg = f"System error occurred while retrieving abstracts for coordinator: {str(exc)}"
        log_audit_event(
            event_type="coordinator_abstracts.list.failed",
            user_id=actor_id,
